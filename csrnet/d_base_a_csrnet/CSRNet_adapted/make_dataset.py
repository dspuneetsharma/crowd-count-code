...
def gaussian_filter_density(gt):
    ...
    for i, pt in enumerate(pts):
        ...
        # Fix: Ensure sigma is valid (positive and reasonable)
        if np.isnan(sigma) or sigma <= 0:
            sigma = 15.0  # Default sigma value
        elif sigma > 100:  # Cap maximum sigma
            sigma = 100.0
            
        density += gaussian_filter(pt2d, sigma, mode='constant')  # Fixed function call
    return density

#set the root to your part_A dataset (current workspace)
