...
def gaussian_filter_density(gt):
    ...
    for i, pt in enumerate(pts):
        ...
        # Adaptive sigma based on y-coordinate (vertical position)
        y_coord = pt[1]  # y-coordinate of the head location
        if 0 <= y_coord < 200:
            sigma = 4
        elif 200 <= y_coord < 500:
            sigma = 10
        elif 500 <= y_coord < 768:
            sigma = 15
        else:
            sigma = 4  # fallback for edge cases
