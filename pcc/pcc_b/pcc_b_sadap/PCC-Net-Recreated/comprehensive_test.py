    ...
    def save_best_worst_images(self):
        ...
        for i, result in enumerate(sorted_results[:10]):
            ...
            if os.path.exists(comparison_path):
                ...
                print(f"  Copied: {filename} (MAE: {result['mae']:.2f})")
            else:
                print(f"  Warning: Comparison image not found for {filename}")
...
def main():
    ...
    # Use the highest epoch model as "best"
    model_path = './exp/PCC_Net09-15_00-17/best_model.pth'
    
    if not os.path.exists(model_path):
        print(f"Error: Model file not found: {model_path}")
        print("Available models:")
        exp_dir = './exp/PCC_Net09-15_00-17'
