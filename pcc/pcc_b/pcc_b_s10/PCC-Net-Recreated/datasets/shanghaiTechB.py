...
class SHT_B(data.Dataset):
    def __init__(self, data_path, main_transform=None, img_transform=None, gt_transform=None):
        ...
        self.seg_path = data_path + '/segmentation'
    ...
    def read_image_and_gt(self,fname):
        ...
        # Handle naming mismatch: IMG_315.jpg -> IMG_315_seg.png
        seg_fname = fname.replace('.jpg', '_seg.png')
        seg = Image.open(os.path.join(self.seg_path, seg_fname))
        # Normalize segmentation from [0, 255] to [0, 1] for binary classification
        seg_array = np.array(seg)
        seg_array = (seg_array > 127).astype(np.uint8)  # Convert 255 to 1, 0 stays 0
