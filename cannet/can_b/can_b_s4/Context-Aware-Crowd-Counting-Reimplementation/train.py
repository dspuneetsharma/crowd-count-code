...
def main():
    ...
    early_stop_threshold = 7.6  # MAE threshold to start early stopping
    patience = 50  # Number of epochs to wait after reaching threshold
    patience_counter = 0
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if best_prec1 <= early_stop_threshold and not early_stop_active:
            early_stop_active = True
            print(f'Early stopping activated! MAE {best_prec1:.3f} <= {early_stop_threshold}')
            print(f'Will stop training if no improvement for {patience} epochs')
        
        if early_stop_active:
            if is_best:
                patience_counter = 0
                print(f'New best MAE: {best_prec1:.3f} - Resetting patience counter (0/{patience})')
            else:
                patience_counter += 1
                print(f'No improvement for {patience_counter}/{patience} epochs (Patience: {patience - patience_counter} remaining)')
                
                if patience_counter >= patience:
                    print(f'Early stopping triggered! No improvement for {patience} epochs')
                    print(f'Best MAE achieved: {best_prec1:.3f}')
                    break
