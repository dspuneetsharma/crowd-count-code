...
def gaussian_filter_density(img,points):
    '''
    Uses a fixed Gaussian kernel (sigma = 10.0) to generate the density map.
...
def process_single_image(img_path):
    ...
    try:
        ...
        points = mat["image_info"][0,0][0,0][0]  # [[x(col), y(row)], ...]
        
        k = gaussian_filter_density(img, points)
        print(f"Image {os.path.basename(img_path)} | pts={len(points)} | sum={k.sum():.3f} | shape={k.shape}")
